...
/* 접속 되었을 때 실행 */
socket_room.on('connect', function() {
  /* 이름을 입력받고 */
  ...
  /* join */
  socket_room.emit('join', {'name':name,'room_num':room_num});
});

/* 서버로부터 데이터 받은 경우 */
socket_room.on('update', function(data) {
...
function send() {
	...
	if(message.length>0){
		...
		socket_room.emit('message', {type: 'message', message: message});
	}
}
