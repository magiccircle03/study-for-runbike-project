...
app.get('/', function(request, response) {
  fs.readFile('./static/chatting4.html', function(err, data) {
    if(err) {
      response.send('에러');
    } else {
...
/* Get 방식으로 /room 경로에 접속하면 실행 됨 */
app.get('/room', function(request, response) {
  fs.readFile('./static/chatting4_room.html', function(err, data) {
    if(err) {
      response.send('에러');
    } else {
      response.writeHead(200, {'Content-Type':'text/html'});
...
app.get('/chat', function(request, response) {
  fs.readFile('./static/chatting4_chat.html', function(err, data) {
    if(err) {
      response.send('에러');
    } else {
      response.writeHead(200, {'Content-Type':'text/html'});
...
socket_chat.on('connection', function(socket) {
  console.log('socket_chat 유저 접속 됨');
  ...
  socket.on('join', function(data) {
      ...
      socket_chat.in(room_num).emit('update', {type: 'join', name: 'SERVER', message: name + '님이 접속하였습니다.'});
  ...
  socket.on('disconnect', function() {
      console.log('['+socket.room_num+'] '+ socket.name + '님이 나가셨습니다.');
...
socket_room.on('connection', function(socket) {
  console.log('socket_room 유저 접속 됨');
  ...
  socket.on('join', function(data) {
      ...
      /* 그 방의 소켓에게 전송 */
      //io.sockets.in(room_num).emit('update', {type: 'join', name: 'SERVER', message:'['+room_num+'] '+ name + '님이 접속하였습니다.'});
      socket_room.in(room_num).emit('update', {type: 'join', name: 'SERVER', message: name + '님이 접속하였습니다.'});
